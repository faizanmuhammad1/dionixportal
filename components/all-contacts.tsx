...
export function AllContacts() {
  ...
  const filteredContacts = contacts.filter(
    (contact) =>
      ...
      false ||
      (contact.form_data && JSON.stringify(contact.form_data).toLowerCase().includes(searchTerm.toLowerCase())),
  )
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <Badge variant="secondary">{filteredContacts.length} Total Contacts</Badge>
          <Badge variant="outline">{filteredContacts.filter((c) => c.status === "pending").length} Pending</Badge>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>All contact form submissions and customer inquiries</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
